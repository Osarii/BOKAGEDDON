...
| 12 | Functional n8n workflow | MANUAL VERIFICATION REQUIRED | `n8n/bonkageddon-run-workflow.json` is valid JSON with Webhook, Code validation, invalid-payload failure response, Code analysis, Switch, three Set actions, and success Respond node. Real execution still requires the user's n8n instance. |
...
- Validation / normalization: PASS, `Validate and Normalize` coerces score, kills, level, duration, outcome, classification, and validation errors.
- Invalid payload blocking: PASS, `Payload Valid?` sends invalid requests to `Invalid Payload Response` instead of normal classification.
- Chained processing/action nodes: PASS, valid payloads follow `Validate and Normalize -> Payload Valid? -> Run Analysis -> Classify Run -> one of three Action nodes -> Respond to Webhook`.
...
Payload Valid?[valid] -> Run Analysis -> Classify Run
Payload Valid?[invalid] -> Invalid Payload Response
...
2. Import the workflow into a real n8n instance, send one valid run, confirm branch selection and success response payload.
3. Send one invalid payload, confirm `Invalid Payload Response` returns `ok: false`.
4. Capture the n8n workflow screenshot for submission.
